...
Two scaffolds:
  A — para-disubstituted benzene   (majority in train + calibration)
  B — 2,5-disubstituted pyridine   (minority in calibration, 100% of test)
...
SCAFFOLD_B = "c1cc(-c2c([*:1])nn3nc([*:2])ccc23)nc(N(c2cccc([*:3])c2))n1"      # 1,3,5-trisubstituted benzene
SCAFFOLD_A = "C1NC([*:1])CN([*:3])C1[*:2]"       # piperidine: C4,C2 + N1 substituted

LOGP_THRESHOLD = 7  # label=1 if MolLogP >= threshold
NUM_BITS = 2048
RADIUS   = 2
SEED     = 42
Noise_A  = 0.5
...
# %%
df_a = feat_df[feat_df["scaffold"] == "A"]
df_b = feat_df[feat_df["scaffold"] == "B"]

print(f"Scaffold A:  {len(df_a)} compounds")
print(f"Scaffold B: {len(df_b)} compounds")
print("\nMolLogP by scaffold:")
...
train_a = df_a.iloc[idx_a[:cut_a]]
calib_a = df_a.iloc[idx_a[cut_a:]]

idx_b  = rng.permutation(len(df_b))
cut_b1 = int(0.50 * len(df_b))
cut_b2 = int(0.60 * len(df_b))
train_b = df_b.iloc[idx_b[:cut_b1]]
calib_b = df_b.iloc[idx_b[cut_b1:cut_b2]]
test_b  = df_b.iloc[idx_b[cut_b2:]]

df_train = pd.concat([train_a, train_b])
df_calib = pd.concat([calib_a, calib_b])
df_test  = test_b.copy()
...
#now umap by colored by split, using the same embedding as above 
_palette = {"train": "grey", "test": "steelblue", "calib": "green", }
fig, ax = plt.subplots(figsize=(8, 6))
for split, col in _palette.items():
    if split == "train":
        _m = feat_df.index.isin(df_train.index)
        ax.scatter(_emb[_m, 0], _emb[_m, 1], s=2, alpha=0.5, color=col, label=f"{split.capitalize()}")

    elif split == "calib":
        _m = feat_df.index.isin(df_calib.index)
        ax.scatter(_emb[_m, 0], _emb[_m, 1], s=2, alpha=0.5, color=col, label=f"{split.capitalize()}", marker="^")
    else:
        _m = feat_df.index.isin(df_test.index)
        ax.scatter(_emb[_m, 0], _emb[_m, 1], s=2, alpha=0.5, color=col, label=f"{split.capitalize()}")
ax.set_xlabel("UMAP 1")
ax.set_ylabel("UMAP 2")
ax.set_title("UMAP of Morgan fingerprints — coloured by split")
ax.legend(markerscale=3)
plt.tight_layout()
plt.show()
plt.close(fig)
...
nc_calib  = ConformalPredictor.get_nonconformity_scores(y_pred_calib, y_calib, 0.5, function="res")
nc_target = ConformalPredictor.get_nonconformity_scores(
    y_pred_test, np.full(len(y_pred_test), 0.5), 0.5, function="res")
...
ax.hist(df_a["MolLogP"], bins=25, alpha=0.6, color="steelblue", label="Scaffold A")
ax.hist(df_b["MolLogP"], bins=25, alpha=0.6, color="tomato",    label="Scaffold B")
